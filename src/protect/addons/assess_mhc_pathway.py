...
def assess_mhc_genes(job, gene_expression, rna_haplotype, univ_options, mhc_genes_options):
    """
    Assess the prevalence of the various genes in the MHC pathway and return a report in the tsv
    format.

    :param toil.fileStore.FileID gene_expression: fsID for the rsem isoform expression file
    ...
    input_files = {
        ...
        'background_file.tsv.tar.gz': mhc_genes_options['background_file']}
    if rna_haplotype is not None:
        input_files['rna_haplotype.sum'] = rna_haplotype
    input_files = get_files_from_filestore(job, input_files, work_dir, docker=False)

    input_files['background_file.tsv'] = untargz(input_files['background_file.tsv.tar.gz'], work_dir)

    # read the background file
    background_df = pd.read_table(input_files['background_file.tsv'], index_col=0, header=0)
    ...
    gene_expressions = {x.split('.')[0]: y for x, y in gene_expressions['TPM'].to_dict().items()}
    # print the report
    ...
    with open('mhc_pathway_report.txt', 'w') as mpr:
        for role in roles:
            ...
            mpr.write(role.center(90, ' '))
            mpr.write("\n")
            mpr.write(
                "\n{:12}{:<12}{:<17}{:<12}{:<20}{:<17}\n".format("Gene", "Observed", "Threshold_GTEX", "Result",
                                                                 "Threshold_TCGA_N", "Result"))
            ...
            if role == 'MHCI loading':
                for mhci_allele in 'HLA_A', 'HLA_B', 'HLA_C':
                    ...
                    mpr.write("{:12}{:<12}{:<17}{:<12}{:<20}{:<17}".format(mhci_allele, 2, num_alleles, result, 'NA', 'NA'))
                ...
                for mhcii_allele in ('HLA_DQA', 'HLA_DQB', 'HLA_DRB'):
                    if rna_haplotype is not None:
                        ...
                        result = num_alleles = 'NA'
                    mpr.write(
                        "{:12}{:<12}{:<17}{:<12}{:<20}{:<17}".format(mhcii_allele, 2, num_alleles, result, 'NA', 'NA'))
            ...
            for ensg in role_df.index:
                ...
                for bkg in b_types:
                    ...
                    result = ('NA' if val == 'NA' else
                              'LOW' if val <= float(gene_expressions[ensg]) else
                              'PASS')
                    b_vals[bkg] = val, result
                mpr.write(
                    "{:12}{:<12}{:<17}{:<12}{:<20}{:<17}".format(ensgName, gene_expressions[ensg], b_vals['gtex'][0],
                                                                   b_vals['gtex'][1],b_vals['tcga'][0],
                                                                   b_vals['tcga'][1]))
